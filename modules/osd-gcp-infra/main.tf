...
resource "google_project_iam_member" "cluster_sa_iam_bindings" {
  for_each = var.enable_gcp_wif_authentication ? toset([]) : toset(var.rh_cluster_sa_roles)
  project  = local.cluster_project
  role     = each.value
  member   = "serviceAccount:${data.google_service_account.cluster_service_account[0].email}"
}

resource "google_project_iam_member" "current_user_iam_bindings" {
  for_each = var.enable_gcp_wif_authentication ? toset([]) : toset(var.rh_cluster_sa_roles)
  project  = local.cluster_project
  role     = each.value
...
resource "google_secret_manager_secret_iam_member" "cluster_sa_secret_bindings" {
  count     = var.enable_gcp_wif_authentication ? 0 : 1
...
resource "google_secret_manager_secret_iam_member" "current_user_cluster_sa_secret_bindings" {
  count     = var.enable_gcp_wif_authentication ? 0 : 1
  project   = google_secret_manager_secret.cluster_sa_keyfile[0].project
  secret_id = google_secret_manager_secret.cluster_sa_keyfile[0].secret_id
  role      = "roles/secretmanager.secretAccessor"
