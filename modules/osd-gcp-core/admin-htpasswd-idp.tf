...
resource "null_resource" "get_default_admin_user_id" {
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      IDP_ID                     = trimspace(data.local_file.default_idp_id.content)
...
resource "null_resource" "cleanup_default_ipd" {
  ...
  provisioner "local-exec" {
    ...
    environment = {
      DEFAULT_IDP_NAME  = local.default_idp_name
      DEFAULT_USER_ID   = data.local_file.default_admin_user_id.content
      DEFAULT_GROUP     = local.default_user_group
      CLUSTER_ID        = trimspace(data.local_file.ocm_cluster_id.content)
      IDP_ID            = trimspace(data.local_file.default_idp_id.content)
...
data "local_file" "default_idp_id" {
  depends_on = [null_resource.get_default_idp_id]
  filename   = local.default_idp_id_file
}

data "local_file" "default_admin_user_id" {
  depends_on = [null_resource.get_default_admin_user_id]
  filename   = local.default_admin_user_id_file
}
