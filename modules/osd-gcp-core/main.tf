...
resource "shell_script" "cluster_install" {
  ...
  environment = {
    ...
    private_service_connect_enabled = var.private_service_connect_enabled
    vpc                             = var.vpc
...
resource "null_resource" "get_cluster_id" {
  ...
  provisioner "local-exec" {
    interpreter = ["/bin/sh", "-c"]
    command     = "ocm get /api/clusters_mgmt/v1/clusters --parameter search=\"name like '$CLUSTER%'\" | jq -re --arg CLUSTER \"$CLUSTER\" '.items[] | select(.name == $CLUSTER) | .id' | head -n1 > $CLUSTER_ID_FILE"
    environment = {
...
resource "null_resource" "get_cluster_details" {
  ...
  provisioner "local-exec" {
    ...
    environment = {
      CLUSTER_ID       = trimspace(data.local_file.ocm_cluster_id.content)
  ...
  provisioner "local-exec" {
    interpreter = ["/bin/sh", "-c"]
    command     = "ocm describe cluster $CLUSTER_ID --json | jq -r '.api.load_balancer_ip' | xargs > $API_SERVER_IP_FILE"
...
data "local_file" "ocm_cluster_id" {
  depends_on = [null_resource.get_cluster_id]
...
resource "google_secret_manager_secret_iam_member" "cluster_details_secret_bindings" {
  count     = var.enable_gcp_wif_authentication ? 0 : 1
  ...
  member    = "serviceAccount:${data.google_service_account.cluster_service_account[0].email}"
}

resource "google_secret_manager_secret_iam_member" "current_user_cluster_details_secret_bindings" {
  count     = var.enable_gcp_wif_authentication ? 0 : 1
  project   = google_secret_manager_secret.cluster_details_secret.project
  secret_id = google_secret_manager_secret.cluster_details_secret.secret_id
  role      = "roles/secretmanager.secretAccessor"
  member    = length(regexall(".iam.gserviceaccount.com$", local.current_user)) > 0 ? format("serviceAccount:%s", local.current_user) : format("user:%s", local.current_user)
}
...
locals {
  ...
  cluster_details = {
    ...
    service_account_name    = var.rh_cluster_sa_name
